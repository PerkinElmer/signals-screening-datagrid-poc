...
var jexcel = (function(el, options) {
    ...
    var defaults = {
        ...
        // Allow comments
        allowComments:false,
		// Allow spread columns
        allowSpreadColumns:false,
        // Allow spread rows
        allowSpreadRows:false,
		// Max number of rows in lazy mode
		maxLazyRows: 10000,
        // Global wrap
        wordWrap:false,
        // CSV source
        ...
        text:{
            ...
            saveAs: 'Save as...',
            about: 'About',
			spreadColumn: 'Spread to column values',
            spreadRows: 'Spread to row values',
    ...
    obj.setData = function(data) {
        ...
        if (obj.options.lazyLoading == true) {
            // Load only obj.options.maxLazyRows records
            var startNumber = 0
            var finalNumber = obj.options.data.length < obj.options.maxLazyRows ? obj.options.data.length : obj.options.maxLazyRows;
    ...
    if (options && options.contextMenu != null) {
        ...
        obj.options.contextMenu = function(el, x, y, e) {
			...
			// Spread columns
            if (obj.options.allowSpreadColumns) {
                items.push({
                    title:obj.options.text.spreadColumn,
                    onclick:function() {
                        let strCell = jexcel.getColumnNameFromId([x,y]);
                        let strValue = obj.getValue(strCell);
                        let strCol = strCell.match(/[a-zA-Z]+/g);
                        const nTotRows = obj.rows.length + 1;
                        for (let iRow = 1; iRow < nTotRows; iRow++){
                            obj.setValue(strCol + iRow.toString(), strValue);
                        }
                    }
                });
            }
            ...
            // Spread rows
            if (obj.options.allowSpreadRows) {
                items.push({
                    title:obj.options.text.spreadRows,
                    onclick:function() {
                        let strCell = jexcel.getColumnNameFromId([x,y]);
                        let strValue = obj.getValue(strCell);
                        let strRow = strCell.match(/\d+/g);
                        const nTotCols = obj.colgroup.length;
                        for (let iCol = 0; iCol < nTotCols; iCol++){
